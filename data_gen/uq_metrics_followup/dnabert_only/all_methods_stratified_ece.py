...
# Methods whose U_total = U_aleatoric + U_epistemic exactly (verified: BALD
# decomposition, sums to 1.0 on both datasets for all of these).
SUM_METHODS = ["mc_dropout", "conv_epinet", "laplace", "ensemble_k5", "ensemble_k3",
               "cnn_mc_dropout", "cnn_ensemble"]
# evidential: U_total == U_aleatoric exactly on boundary (U_epistemic ~ 0,
# canonical single-Dirichlet-mean formulation, see nn_proj/models/evidential/
# evidential.py:predict_evidential); AliSim's U_epistemic for evidential is a
# nonzero, differently-scaled quantity (some Dirichlet-sampling extension not
# in the canonical implementation) that does NOT sum with U_aleatoric to a
# normalized total -- so use U_aleatoric alone as the entropy source for
# evidential on both datasets, not the sum.
ALEATORIC_ONLY_METHODS = ["evidential"]
# RF's boundary column is "rf", AliSim's is "rf_kmer" -- same method, verified
# same BALD-sum-to-1 property on both.
...
def main():
    ...
    for m in alisim_methods:
        ...
        pooled_rows.append(metrics_row(p1_m, pr, lb, dataset="promoter_alisim_dense", method=method_label, stratum="pooled"))
        for t in sorted(set(blm)):
            mm = blm == t
            strat_rows.append(metrics_row(p1_m[mm], pr[mm], lb[mm], dataset="promoter_alisim_dense", method=method_label, stratum=t))
    ...
    pooled.to_csv("/home/glh52/.claude/jobs/80f6ac1e/tmp/pooled_ece_allmethods.csv", index=False)
    strat.to_csv("/home/glh52/.claude/jobs/80f6ac1e/tmp/stratified_ece_allmethods.csv", index=False)
