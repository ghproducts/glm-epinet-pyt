"""Second non-pretrained baseline: a small from-scratch 1D-CNN over one-hot
DNA, in the DeepBind/DeepSEA lineage (conv over one-hot bases -> global
pooling -> dense head) -- the standard non-pretrained *neural* baseline for
sequence classification, predating GLM transfer learning in this space.

Unlike the k-mer + Random Forest baseline (decomp_compare_*_rf.py), this one
is a genuine neural network, so it supports exactly the same UQ-sampling
mechanisms already used on DNABERT2: MC dropout (one trained model, dropout
active at eval, K stochastic passes) and deep ensembles (K independently
random-initialized-and-trained models, no pretraining shared between them
at all -- a stronger "genuinely independent" condition than the DNABERT2
ensembles, which all start from the same pretrained checkpoint).

No HF Trainer, no tokenizer, no pretrained weights anywhere in this file.
"""
