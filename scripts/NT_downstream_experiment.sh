...
#MODEL="hyenaDNA" # DNABERT2, NT_transformer, hyenaDNA, CARMANIA
CKPT_ROOT="trained_models_${SEED}"
OUT_ROOT="inference_results_${SEED}"
BATCH_SIZE=8

# training params
TRAINED=True #Skips training if true.
LR=1e-6
EPOCHS=5

# testing params
UQ_method="mc_dropout" # base, mc_dropout
K_SAMPLES=10

mkdir -p "$CKPT_ROOT"
mkdir -p "$OUT_ROOT"

MODELS="CARMANIA hyenaDNA NT_transformer DNABERT2"
...
do
    ...
    if [ "$TRAINED" = True ] ; then
        ...
        for ENTRY in "${TRAIN_TASKS[@]}"; do
            ...
            python -m nn_proj.models.${MODEL}.train_base \
