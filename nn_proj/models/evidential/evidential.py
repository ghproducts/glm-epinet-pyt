...
2018, https://arxiv.org/abs/1806.01768). Not post-hoc -- changes both the
output activation and the training loss, so (unlike epinet) each backbone
needs its own `train_evidential.py`; see
nn_proj/models/DNABERT2/train_evidential.py.

The network's non-negative "evidence" e >= 0 per class parameterizes a
Dirichlet over the probability simplex (alpha = e + 1); predicted
probability is the Dirichlet mean alpha_c/S, S = sum(alpha), and vacuity
K/S is the uncertainty signal (S grows only with accumulated evidence).
Loss is the expected sum-of-squares between the one-hot label and the
Dirichlet-distributed probabilities (eq. 3/5; the digamma/CE variant, eq.
4, is available via `loss_type="ce"`), plus a KL term shrinking evidence
for non-true classes, ramped in over `annealing_step` epochs to avoid
suppressing all evidence before the network has learned anything.
"""
